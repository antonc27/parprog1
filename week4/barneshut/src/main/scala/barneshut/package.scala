...
package object barneshut {
  ...
  private def isInside(q: Quad, b: Body): Boolean = {
    val minX = q.centerX - q.size / 2
    val maxX = minX + q.size
    val minY = q.centerY - q.size / 2
    val maxY = minY + q.size
  ...
  case class Fork(
    ...
    def insert(b: Body): Fork = {
      if (!isInside(this, b)) {
        ...
        def quadInsert(q: Quad): Quad = if (isInside(q, b)) q.insert(b) else q
        Fork(quadInsert(nw), quadInsert(ne), quadInsert(sw), quadInsert(se))
      }
    }
  }
  ...
  extends Quad {
    ...
    def insert(b: Body): Quad = {
      if (size > minimumSize) {
        ...
        Leaf(centerX, centerY, size, bodies ++ List(b))
      }
    }
  }
  ...
  class SectorMatrix(val boundaries: Boundaries, val sectorPrecision: Int) extends SectorMatrixInterface {
    ...
    def +=(b: Body): SectorMatrix = {
      ...
      val xSector = ((x - boundaries.minX) / sectorSize).toInt
      val ySector = ((y - boundaries.minY) / sectorSize).toInt
      apply(xSector, ySector) += b
      this
    }
